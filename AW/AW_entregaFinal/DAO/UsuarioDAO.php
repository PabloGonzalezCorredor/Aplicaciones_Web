...
class UsuarioDAO extends DAO
{
    ...
    public function buscaUsuario($nombreUsuario)
    {
        ...
        if ($rs && count($rs) == 1) {
            ...
            $result = new Usuario($fila['imagen'],$fila['nombreUsuario'], $fila['password'], $fila['nombre'], $fila['rol'], $fila['privacidad'], $fila['edad'], $fila['genero'], $fila['id']);
        } 
        return $result;
    }
    ...
    public function buscaUsuarios($cadena)
    {
        ...
        if ($rs && count($rs) > 0) { 
            foreach ($rs as $fila) { 
                $usuario = new Usuario($fila['imagen'],$fila['nombreUsuario'], $fila['password'], $fila['nombre'], $fila['rol'], $fila['privacidad'],$fila['edad'], $fila['genero'], $fila['id']);
                $result[] = $usuario; 
            }
        }
    ...
    {
        $query="INSERT INTO usuarios(imagen, nombreUsuario, nombre, password, rol, privacidad, edad, genero) VALUES 
        (
            ...
            '". mysqli_real_escape_string($this->mysqli, $usuario->getPrivacidad()) . "',
            '". mysqli_real_escape_string($this->mysqli, $usuario->getEdad()) . "',
            '". mysqli_real_escape_string($this->mysqli, $usuario->getGenero()) . "'
        )";
    ...
    {
        $query="UPDATE usuarios U SET imagen = '". mysqli_real_escape_string($this->mysqli, $usuario->getImagen()) . "', nombreUsuario = '". mysqli_real_escape_string($this->mysqli, $usuario->getNombreUsuario()) . "', nombre='". mysqli_real_escape_string($this->mysqli, $usuario->getNombre()) . "', password='". mysqli_real_escape_string($this->mysqli, $usuario->getPassword()) . "', rol='". mysqli_real_escape_string($this->mysqli, $usuario->getRol()) . "', privacidad='". mysqli_real_escape_string($this->mysqli, $usuario->getPrivacidad()) . "', edad='". mysqli_real_escape_string($this->mysqli, $usuario->getEdad()) . "', genero='". mysqli_real_escape_string($this->mysqli, $usuario->getGenero()) . "' WHERE U.id='". mysqli_real_escape_string($this->mysqli, $usuario->getId()) . "'";
        $rs = $this->ejecutarComando($query);
    }

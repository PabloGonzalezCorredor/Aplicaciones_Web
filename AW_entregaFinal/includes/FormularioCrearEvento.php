...
class FormularioCrearEvento extends Formulario
{
    ...
    protected function generaCamposFormulario(&$datos)
    {
        ...
        <div class="form">
                ...
                <div style="display: flex;">
                    ...
                    <div class="tarifa-input-container1">
                        <div class="tarifa-input-container2">
                            <input id="cantidad" type="number" placeholder="Cantidad" name="cantidad[]"/>
                            <input id="consumiciones" type="number" placeholder="Consumiciones" name="consumiciones[]" />
                            <input id="precio" type="number" placeholder="Precio" name="precio[]"/>
                        </div>
            ...
            <div style="display: flex; width: 100%; justify-content: center">
                <button type="submit" class="button" name="registrar"><h4>Crear Evento</h4></button>
            </div>
        </div>
        HTML;
        return $html;
    ...
    protected function procesaFormulario(&$datos)
    {
        ...
        foreach ($cantidades as $cantidad) {
            ...
            if ($cantidad !== '' && $cantidad !== null && $cantidad >= 0) {
                ...
                $this->errores['tarifas'] = 'No deje huecos en blanco';
            }
        }
        ...
        if (count($preciosFiltrados) === 0 || count($consumicionesFiltradas) === 0 || count($cantidadesFiltradas) === 0) {
            $this->errores['tarifas'] = 'Debe proporcionar al menos una tarifa';
        } else {
            // Procesamos cada tarifa
            $tarifas = [];
            for ($i = 0; $i < count($preciosFiltrados); $i++) {
                $cantidad = trim($cantidadesFiltradas[$i]);
        ...
        if (count($this->errores) === 0) {
            ...
            if ($existe) {
                ...
                EventoSA::creaEvento($_SESSION['id'], $imagenBinaria, $nombreEvento, $fecha, $horaIni, $horaFin, $localizacion, $tarifas);
                $evento = EventoSA::obtenerEvento($nombreEvento, $fecha);
                $seguidores = UsuarioSA::obtenerSeguidores($_SESSION['id']);

                $idEvento = $evento->getId();

                foreach ($seguidores as $seguidor){
                    NotificacionSA::notificarEvento($seguidor->getId(), $idEvento);
                }
            }
        }
    }
...
<script>
    document.addEventListener("DOMContentLoaded", function() {
        ...
        agregarTarifaBtn.addEventListener("click", function() {
            ...
            nuevaTarifa.innerHTML = `
                ...
                <div class="tarifa-input-container1">
                    <div class="tarifa-input-container2">
                        ...
                        <input id="consumiciones" type="number" placeholder="Consumiciones" name="consumiciones[]" />
                        <input id="precio" type="number" placeholder="Precio" name="precio[]"/>
                        <button class="delete-tarifa-button"><h4>-</h4></button>
                    </div>
                    <input id="informacion" type="text" placeholder="Información"  name="informacion[]" />
                </div>
            `;
            ...
            const eliminarTarifaBtn = nuevaTarifa.querySelector(".delete-tarifa-button");
